...
export class Store {
    ...
    let storeCreator = (mainModule: MainModule) => {
      ...
      store.subscription = action$.pipe(
        ...
        concatMap(async ({count, action}: any) => (count === 1) ? (console.log("%cYou are using ActionStack. Happy coding! 🎉", "font-weight: bold;"),
          await store.updateState("@global", async () => store.setupReducer(), action)) : action),
        store.processAction()
      ).subscribe();
   ...
   * @protected
   */
  protected applyChange(initialState: any, { path, value }: { path: string[], value: any}, edges: Tree<boolean>) {
    let currentState = Object.keys(edges).length > 0 ? initialState : { ...initialState };
   ...
   * @template T
   */
  protected async setState<T = any>(slice: keyof T | string[], value: any, action: Action<any> = systemActions.updateState()): Promise<any> {
    let newState: any;
   ...
   * @template T
   */
  protected async updateState<T = any>(slice: keyof T | string[], callback: AnyFn, action: Action<any> = systemActions.updateState()): Promise<any> {
    ...
    return await this.setState(slice, result, action);
  }
      ...
      for (const [reducer, path] of reducerMap) {
        try {
          ...
          if (currentState !== updatedState) {
            state = await this.applyChange(state, { path, value: updatedState }, modified);
          }
        } catch (error: any) {
  ...
  protected async setupReducer(state: any = {}): Promise<any> {
    // Build feature reducers
    let featureReducers = [{ slice: this.mainModule.slice!, reducer: this.mainModule.reducer }, ...this.modules]
    ...
    // Set the reducer in the pipeline
  ...
  protected applyMiddleware(): Store {
    ...
    // Assign the composed dispatch function to store's dispatch method
